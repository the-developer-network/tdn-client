...
export function ArticleCard({
    ...
    return (
        <article
            ...
            className="flex cursor-pointer flex-col gap-3 border-b border-ink/10 p-4 transition-colors hover:bg-ink/[0.03]"
        >
            ...
            <div className="flex gap-4">
                <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 line-clamp-3 text-sm leading-6 text-ink/50">
                        {excerpt}
                    </p>
                </div>
                {cover && (
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-ink/40">
                ...
                {tags.slice(0, 3).map((tag) => (
                    <span
                        key={tag.name}
                        className="rounded-full bg-ink/5 px-2 py-0.5 text-ink/50"
                    >
                        #{tag.name}
                    </span>
                ))}
