...
Every route except `/onboarding` sits under a pathless layout route rendering `OnboardingGate` (`src/app/OnboardingGate.tsx`), which sends an account whose `followingCount` is 0 to `/onboarding` to pick its fields and follow five accounts. It stands down while the auth modal is open — redirecting mid `verify-email` would unmount the modal with the page — and passes rather than redirects when the profile request fails. **`e2e/fixtures.ts` seeds `tdn-onboarding`**; without it every authenticated spec would land on `/onboarding`.
